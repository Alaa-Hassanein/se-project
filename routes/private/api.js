...
module.exports = function(app) {
  ...
  app.post('/manage/grade', async function(req, res) {
    const request = {
    ...
    try {
      const requ = await db('se_project.enrollments').insert(request);
      return res.status(200).json(requ);
    } catch (e) {
      console.log(e.message);
      return res.status(400).send('Could not upload grades');
    }
  });
